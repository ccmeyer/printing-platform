...
class EndTypeParams(Structure):
    """
    _pack_ 定义类型的内存对齐数，对应于c中的#pragma pack(1)
    _pack_ field specify the memory alignment of the current object
    """
    _pack_ = 1
    _fields_ = [
        ("xBias", c_float),
...
def load():
    if platform.system() == "Windows":
        ...
        dllabspath=os.path.dirname(os.path.abspath(filename))+os.path.sep+"DobotDllM1.dll"
        os.chdir(os.path.dirname(os.path.abspath(filename))+os.path.sep)
        return CDLL(dllabspath,  RTLD_GLOBAL) 
    else:
        print("Your system is not supported by Dobot yet")
        return
