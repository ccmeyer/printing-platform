...
class Platform(Robot.Robot, Arduino.Arduino, Regulator.Regulator):
    ...
    def __init__(self):
        ...
        self.startup_mode = 'droplet' if self.ask_yes_no(message='Print in droplet mode? (y/n)') else 'p1000'
        self.sim = False
        self.show_arduino_output = False
        ...
        self.balance_debug = True
        self.balance_last_raw = ''
        self.balance_parse_errors = 0
        self.balance_success_reads = 0
        self.balance_last_success_time = None
        self.balance_recent_raw = []
    ...
    def read_defaults(self):
        ...
        self.mode = None
        self.select_mode(mode_name=self.startup_mode)
        return

    def normalize_calibration_data(self):
        alias_pairs = [
            ('home', 'home_position'),
        ...
        for canonical, legacy in alias_pairs:
            if canonical not in self.calibration_data and legacy in self.calibration_data:
                self.calibration_data[canonical] = self.calibration_data[legacy]
            if legacy not in self.calibration_data and canonical in self.calibration_data:
                self.calibration_data[legacy] = self.calibration_data[canonical]
        if 'array_start' in self.calibration_data and 'pause' not in self.calibration_data:
            self.calibration_data['pause'] = self.calibration_data['array_start']
        if 'pause' in self.calibration_data and 'array_start' not in self.calibration_data:
            self.calibration_data['array_start'] = self.calibration_data['pause']
        return

    def get_dobot_calibrations(self):
        target_name = 'Calibrations/print_calibrations_{}_{}_v2.json'.format(self.default_settings['robot_type'], self.mode)
        self.calibration_file_path = self.get_file_path(target_name, base=True)
        with open(self.calibration_file_path) as json_file:
            self.calibration_data = json.load(json_file)
        self.normalize_calibration_data()
        print('Loaded trip printing calibration: {}_{}_v2'.format(self.default_settings['robot_type'], self.mode))
    ...
    def get_plate_data(self):
        target_name = 'Calibrations/{}_{}_{}_v2.json'.format(self.default_settings['plate_type'], self.default_settings['robot_type'], self.mode)
        self.plate_file_path = self.get_file_path(target_name, base=True)
        with open(self.plate_file_path) as json_file:
            self.plate_data = json.load(json_file)
        print('Loaded trip plate calibration: {}_{}_{}_v2'.format(self.default_settings['plate_type'], self.default_settings['robot_type'], self.mode))
    ...
    def load_dispenser_defaults(self,mode):
        ...
        self.target_disp_volume = self.default_settings['dispenser_types'][mode].get('target_disp_volume')
    ...
    def adjust_target_disp_volume(self, change):
        if self.mode != 'droplet' or self.target_disp_volume is None:
            return False

        min_target = 0.06
        max_target = 0.08
        new_target = round(self.target_disp_volume + change, 3)
        new_target = min(max(new_target, min_target), max_target)

        if new_target == self.target_disp_volume:
            print(f'Target droplet volume remains at {self.target_disp_volume:.3f} uL ({self.target_disp_volume * 1000:.0f} nL)')
            return True

        self.target_disp_volume = new_target
        print(f'Target droplet volume: {self.target_disp_volume:.3f} uL ({self.target_disp_volume * 1000:.0f} nL)')
        return True

    def confirm_target_disp_volume(self):
        if self.mode != 'droplet' or self.target_disp_volume is None:
            return True

        print(f'Current target droplet volume: {self.target_disp_volume:.3f} uL ({self.target_disp_volume * 1000:.0f} nL)')
        if not self.ask_yes_no(message='Do you want to change the target droplet volume? (y/n)'):
            return True

        print('Use ; to increase by 0.005 uL and . to decrease by 0.005 uL.')
        print('Press q when the target droplet volume is correct.')
        while True:
            key = self.get_current_key()
            if key == ';':
                self.adjust_target_disp_volume(0.005)
            elif key == '.':
                self.adjust_target_disp_volume(-0.005)
            elif key == Key.esc:
                continue
            elif key == 'q':
                if self.ask_yes_no(message='Use this target droplet volume for calibration? (y/n)'):
                    return True
                print('Continue adjusting the target droplet volume.')
            else:
            ...
            try:
                if self.balance.in_waiting > 0:
                    ...
                    try:
                        ...
                        self.balance_last_raw = data.strip()
                        self.balance_recent_raw.append(self.balance_last_raw)
                        if len(self.balance_recent_raw) > 5:
                            self.balance_recent_raw = self.balance_recent_raw[-5:]
                        ...
                        self.balance_success_reads += 1
                        self.balance_last_success_time = time.time()
                    except Exception:
                        self.balance_parse_errors += 1
                        continue
    ...
    def get_array_start_coords(self):
        if 'pause' in self.calibration_data:
            return self.calibration_data['pause']

        x, y = self.correct_xy_coords(0, -0.5 * self.spacing)
        z = self.top_left['z'] - (0.5 * self.col_z_step)
        return {'x': x, 'y': y, 'z': z}

    def move_to_array_start(self):
        target_coords = self.get_array_start_coords()
        print('Moving to array start position')
        self.move_dobot(self.current_coords['x'], self.current_coords['y'], self.height)
        self.move_dobot(target_coords['x'], target_coords['y'], self.height)
    ...
    def print_array(self):
        ...
        all_exp = self.get_all_paths('Print_arrays/eAA*/',base=True)
        experiment_folder,quit = select_options(all_exp,message='Select one of the eAA experiments:',trim=True)
        if quit: return
    ...
    def drive_platform(self):
        ...
        while True:
            ...
            #     self.close_valves()
                ...
                if not self.adjust_target_disp_volume(0.005):
                ...
                self.print_balance_debug_snapshot()

            elif key == Key.esc:
                continue
            elif key == "q":
                if self.ask_yes_no(message='Do you want to quit the entire platform? (y/n)'):
                    if self.ask_yes_no(message='Are you sure? (y/n)'):
                        print('Quitting...')
                        section_break()
                        return
                    else:
                        print("Stayed in the platform")
                else:
                    print("Stayed in the platform")
            else:
                print('Please enter a valid key, not:',key)
    ...
    def print_droplets(self,freq,pulse_width,refuel_width,count,aspiration=False):
        ...
        i = 0
        pressure_fault_prompted = False
        while self.correct_pressure == False:
            verbose = False
            if i % 10 == 0:
                print('Pressure is incorrect')
                if i == 0:
                    print('Press Backspace to abort this print and return to the platform without quitting the program.')
                verbose = True
            time.sleep(0.2)
            self.compare_pressures(verbose=verbose)
            if self.check_for_pause():
                if self.ask_yes_no(message="Abort this print and return to the platform? (y/n)"):
                    print('Aborting print and returning control to the platform\n')
                    return False
                if self.ask_yes_no(message="Print Anyway? (y/n)"):
                    print('Continuing with print...\n')
                    self.correct_pressure = True
            if not pressure_fault_prompted and i >= 25 and self.correct_pressure == False:
                pressure_fault_prompted = True
                print('Persistent pressure fault detected.')
                if self.ask_yes_no(message="Abort this print and return to the platform? (y/n)"):
                    print('Aborting print and returning control to the platform\n')
                    return False
                print('Continuing to wait for pressures to recover. Press Backspace any time to abort this print.')
            print('.',end='')
            i += 1
    ...
    def control_printing(self):
        ...
        while True:
            ...
            if key == '1':
                ...
                if self.ask_yes_no(message='Move to loading position and open the gripper? (y/n)'):
                    self.move_to_location(location='loading')
                    self.load_gripper()
                    print('Returned to pressure control. Move back to balance with [ or ] when ready.')
            elif key == Key.esc:
                continue
            elif key == "q":
    ...
    def get_balance_debug_diagnosis(self):
        if self.balance_success_reads == 0:
            return 'No successful balance reads yet. Check COM port, cable, scale output mode, and whether the scale is powered on.'

        if self.balance_last_success_time is not None and (time.time() - self.balance_last_success_time) > 5:
            return 'Balance data appears to have stopped arriving. Check the serial connection, COM port ownership, and whether the scale is still streaming.'

        if self.balance_parse_errors > self.balance_success_reads:
            return 'Balance data is arriving but many lines are failing to parse. Check for a changed output format, units, or unstable serial text.'

        if self.mass == 'unknown':
            return 'Balance thread is running but no usable mass is being retained. Check for intermittent serial drops or parsing failures.'

        if self.stable_mass == 'not_stable':
            return 'Balance is reading, but the signal is drifting too much to pass the stability rule. Check for tubing drag, airflow, vibration, or pressure-induced motion on the balance.'

        return 'Balance data looks healthy and stable.'

    def print_balance_debug_snapshot(self):
        age = 'never'
        if self.balance_last_success_time is not None:
            age = round(time.time() - self.balance_last_success_time, 1)
        print('\n--- Balance debug snapshot ---')
        print('Current mass:', self.mass)
        print('Stable mass:', self.stable_mass)
        print('Mass diff metric:', self.mass_diff)
        print('Recent parsed masses:', self.masses if hasattr(self, 'masses') else [])
        print('Recent raw balance lines:', self.balance_recent_raw)
        print('Last raw balance line:', self.balance_last_raw)
        print('Successful balance reads:', self.balance_success_reads)
        print('Balance parse errors:', self.balance_parse_errors)
        print('Seconds since last successful read:', age)
        print('Likely diagnosis:', self.get_balance_debug_diagnosis())
        print('------------------------------')
        return
    ...
    def wait_for_stable_mass(self):
        ...
        while True:
            if self.stable_mass != 'not_stable':
                ...
                count = 0
                print('.',end='')
                if self.balance_debug and not debug_reported and (time.time() - start_time) > 20:
                    debug_reported = True
                    self.print_balance_debug_snapshot()
            if count == 10:
    ...
    def calibrate_printer_head(self, target=None,tolerance=0.05):
        ...
        while True:
            ...
            self.test_print(droplets=droplet_count)
