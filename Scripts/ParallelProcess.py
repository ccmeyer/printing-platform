...
def parse_measurement(d):
    # eg: S     0.0000 g
    # s = 'S     0.0000 g'
    sc, m = d[:-11], d[-11:]
    return float(m.strip()[:-2])
...
def read_balance(s):
    if s.in_waiting > 0:
        ...
        try:
            ...
            time.sleep(0.01)
            return read_balance(s)
        return parse_measurement(data)


def balance_tracker(queue, storage, s):
    while True:
        # Establish connection with the balance
        # while True:
        #     s = serial.Serial(port='COM7', baudrate=9600, bytesize=8, timeout=2, stopbits=serial.STOPBITS_ONE)
        #
        #     # try:
        #     #     s = serial.Serial(port='COM7', baudrate=9600, bytesize=8, timeout=2, stopbits=serial.STOPBITS_ONE)
        #     #     data = ""
