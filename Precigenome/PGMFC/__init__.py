...
class PGMFC(object):
    ...
    @property
    def serial(self):
        c_error, serial_number = controller.mfcs_get_serial(self.__handle)
    ...
    def purge_on(self, channel_number):
        """start pump"""
        c_error = controller.mfcs_purge_on(self.__handle, channel_number)
        utils.parse_error(c_error)

    def purge_off(self, channel_number):
        """stop pump"""
        c_error = controller.mfcs_purge_off(self.__handle, channel_number)
        utils.parse_error(c_error)
    ...
    def queryRotaryAddress(self, switchType, mountID):
        ...
        return controller.mfcs_queryRotaryAddress(self.__handle, switchType, mountID)

    def rotaryReset(self, switchType, mountID):
        """reset"""
        return controller.mfcs_rotaryReset(self.__handle, switchType, mountID)

    def queryRotaryCurPos(self, switchType, mountID):
        """query current pos"""
        return controller.mfcs_queryRotaryCurPos(self.__handle, switchType, mountID)

    def rotarySwitchTo(self, switchType, mountID, portID):
        """switch to X"""
        return controller.mfcs_rotarySwitchTo(self.__handle, switchType, mountID, portID)
    ...
    class Channel(object):
        ...
        def purge_on(self):
            """start pump"""
            return self.__instrument.purge_on(self.__channel_number)

        def purge_off(self):
            """stop pump"""
            return self.__instrument.purge_off(self.__channel_number)
